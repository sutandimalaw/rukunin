...
let DuesService = class DuesService {
    ...
    async getSummary(period) {
        ...
        const totalExpected = billings.reduce((sum, b) => sum + Number(b.amount), 0);
    ...
    async pay(id, dto, userId) {
        return this.prisma.$transaction(async (tx) => {
            ...
            const kepalaKeluarga = billing.household.members[0]?.fullName ?? billing.household.kkNumber;
            const description = `Iuran warga - ${kepalaKeluarga} - Periode ${billing.period}`;
            const transaction = await this.financeService.createWithTx(tx, {
                type: 'IN',
                category: 'iuran',
    ...
    async getDelinquent(minMonths, lookbackMonths) {
        const periods = [];
        const now = new Date();
    ...
    async getMyDues(userEmail) {
        ...
        if (!resident) {
            throw new common_1.NotFoundException('Data warga tidak ditemukan. Pastikan email kamu terdaftar di data RT.');
        }
        const billings = await this.prisma.duesBilling.findMany({
    ...
    async requestPay(dto) {
        ...
        if (result.count === 0) {
            throw new common_1.BadRequestException('Tidak ada tagihan yang dapat diajukan (pastikan status masih UNPAID)');
        }
        return {
    ...
    async rejectPay(id) {
        ...
        if (billing.status !== 'PENDING') {
            throw new common_1.BadRequestException('Hanya tagihan berstatus PENDING yang dapat ditolak');
        }
        return this.prisma.duesBilling.update({
            where: { id },
            data: {
